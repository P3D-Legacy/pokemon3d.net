        ...
        <div class="flex justify-end gap-1">
            @auth
                ...
                @if(auth()->user()->id != $resource->user_id)
                    <a href="{{ route('resource.rate', $resource->uuid) }}" class="px-2 py-1 transition-colors duration-150 rounded-md text-sky-100 bg-sky-600 focus:shadow-outline hover:bg-sky-700">
                ...
                @if(auth()->user()->id == $resource->user_id)
                    <div x-data="{ dropdownOpen: false }" class="" x-cloak>
                        ...
                        <div x-show="dropdownOpen" @click.away="dropdownOpen = false" class="absolute z-10 w-48 py-2 mt-2 bg-white border border-slate-200 rounded-md shadow-md dark:bg-slate-900 dark:border-slate-800">
                            <a href="{{ route('resource.update', $resource->uuid) }}" class="flex items-center w-full px-4 py-2 text-sm text-left text-slate-700 dark:text-slate-300 hover:bg-green-700 hover:text-white">
                            ...
                            <a href="{{ route('resource.edit', $resource->uuid) }}" class="flex items-center w-full px-4 py-2 text-sm text-left text-slate-700 dark:text-slate-300 hover:bg-green-700 hover:text-white">
                                <svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                </svg>
                                @lang('Edit')
                            </a>
                            <a href="{{ route('resource.delete', $resource->uuid) }}" class="flex items-center w-full px-4 py-2 text-sm text-left text-slate-700 dark:text-slate-300 hover:bg-red-700 hover:text-white">
                                <svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
