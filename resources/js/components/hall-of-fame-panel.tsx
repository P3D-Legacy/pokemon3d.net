...
export function HallOfFamePanel({ entries }: HallOfFamePanelProps) {
    if (entries.length === 0) {
        return <p className="text-sm text-muted-foreground">No Hall of Fame entries yet</p>;
    }
    ...
    return (
        <div className="flex w-full min-w-0 flex-col gap-8">
            <div className="flex flex-col gap-1">
                ...
                <div className="text-2xl font-bold">{entries.length} entries</div>
            </div>

            {entries.map((entry) => (
                <div key={entry.id} className="flex flex-col gap-3 border border-border bg-muted/10 p-4">
                    <div className="flex flex-wrap items-center gap-2">
                        <Badge variant="default">Entry No. {entry.id + 1}</Badge>
                    ...
                    {entry.pokemon.length > 0 ? (
                        ...
                        <p className="text-sm text-muted-foreground">No Pokémon recorded</p>
                    )}
                </div>
            ))}
        </div>
