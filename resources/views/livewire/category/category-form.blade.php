<x-modal>
    ...
    <x-slot name="content">
        ...
        <x-jet-label for="parent" class="mt-4" value="{{ __('Parent category') }}" />
        <div class="relative inline-block w-full">
            <select class="w-full h-10 pl-3 pr-6 text-base text-gray-800 placeholder-gray-600 border rounded-lg appearance-none focus:shadow-outline" id="parent" name="parent" wire:model.defer="parent">
                <option value="">@lang('---')</option>
                @foreach ($categories as $c)
                    <option value="{{ $c->id }}" {{ $c->id == $category->parent_id ? 'selected="selected"' : '' }}>{{ $c->name }}</option>
                @endforeach
            </select>
