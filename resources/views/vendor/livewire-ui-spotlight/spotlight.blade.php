<div>
    ...
    <div x-data="LivewireUISpotlight({ componentId: '{{ $this->id }}', placeholder: '{{ trans('livewire-ui-spotlight::spotlight.placeholder') }}', commands: {{ $commands }} })"
        ...
        <div x-show="isOpen" @click="isOpen = false" x-transition:enter="ease-out duration-200" x-transition:enter-start="opacity-0"
            ...
            <div class="absolute inset-0 bg-black dark:bg-white opacity-25"></div>
        </div>
        ...
        <div x-show="isOpen" x-transition:enter="ease-out duration-200"
             ...
             class="relative bg-slate-900 rounded-lg overflow-hidden shadow-xl transform transition-all max-w-lg w-full">
            <div class="relative">
                ...
                <input @keydown.tab.prevent="" @keydown.prevent.stop.enter="go()" @keydown.prevent.arrow-up="selectUp()"
                       ...
                       class="appearance-none w-full bg-transparent px-6 py-4 text-slate-300 text-lg placeholder-slate-500 focus:border-0 focus:border-transparent focus:shadow-none outline-none focus:outline-none"
                       x-bind:placeholder="inputPlaceholder">
            </div>
            <div class="border-t border-slate-800" x-show="filteredItems().length > 0" style="display: none;">
                    ...
                    <template x-for="(item, i) in filteredItems()" :key>
                        <li>
                            <button @click="go(item[0].item.id)" class="block w-full px-6 py-3 text-left"
                                    :class="{ 'bg-slate-700': selected === i, 'hover:bg-slate-800': selected !== i }">
                                <span x-text="item[0].item.name"
                                      :class="{'text-slate-300': selected !== i, 'text-white': selected === i }"></span>
                                <span x-text="item[0].item.description" class="ml-1"
                                      :class="{'text-slate-500': selected !== i, 'text-slate-400': selected === i }"></span>
                            </button>
                        </li>
                    </template>
                </ul>
