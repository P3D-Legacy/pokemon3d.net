<x-guest-layout>
	<x-jet-authentication-card>
		...
		@if(\Carbon\Carbon::createFromFormat('Y-m-d', '2022-05-01')->isFuture())
			<div class="px-4 py-3 text-sm leading-normal text-blue-700 bg-blue-100 rounded-lg" role="alert">
				<p>
					...
					This new website now has its own authentication. Please <a class="font-bold hover:underline" href="{{ env('LOGIN_AUTH_BLOG_POST') ?? route('blog.index') }}">read this blog post</a> for more information!
				</p>
			</div>
        ...
        @if(!(config('services.xenforo.api_key') and config('services.xenforo.api_url')) and config('services.gamejolt.private_key') and !(config('services.discord.client_id') and config('services.discord.client_secret')) and !(config('services.twitter.client_id') and config('services.twitter.client_secret')) and !(config('services.facebook.client_id') and config('services.facebook.client_secret')) and !(config('services.twitch.client_id') and config('services.twitch.client_secret')))
            ...
            @if(config('services.gamejolt.private_key'))
                <button class="flex items-center justify-center w-full px-4 py-3 mt-2 text-sm font-semibold tracking-widest text-black uppercase transition border border-transparent rounded-md bg-gamejolt-green dark:bg-gamejolt-dark-green hover:bg-opacity-70 focus:outline-none focus:border-green-200 focus:ring focus:ring-green-100 disabled:opacity-25" type="button" onclick="toggleModal('gamejolt')">
                    <img src="{{ asset('img/gamejolt-logo-light-1x.png') }}">
                </button>
            @endif

            <div class="grid grid-flow-col gap-2 auto-cols-auto">
                @if(config('services.discord.client_id') and config('services.discord.client_secret'))
                ...
                @endif
                @if(config('services.twitch.client_id') and config('services.twitch.client_secret'))
