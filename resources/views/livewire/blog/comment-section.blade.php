<div class="w-full p-6 pt-4 mt-6 mb-10 overflow-hidden bg-white rounded-lg shadow-md sm:max-w-3xl dark:bg-slate-900 border border-slate-200 dark:border-slate-700">
    ...
    <div class="mb-6">
        ...
        @auth
            <button onclick="Livewire.emit('openModal', 'blog.comment-modal', {{ json_encode(['post' => $post->id, 'parentComment' => null]) }})" class="px-2 py-1 text-sm font-bold text-white bg-slate-500 rounded hover:bg-slate-700"><svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" /></svg> @lang('Comment')</button>
        @endauth
        @guest
