...
class WikiHelper
{
    ...
    public static function getSearchResults(string $query)
    {
        ...
        $endpoint = 'action=query&format=json&list=search&indexpageids=1&iwurl=1&srsearch='.$query.'&srnamespace=0&srprop=size%7Cwordcount%7Ctimestamp%7Csnippet&srsort=relevance';
        return self::sendRequest($endpoint);
    }

    public static function getAllPages(): Collection
    {
        $endpoint = 'action=query&format=json&list=allpages&indexpageids=1&iwurl=1&apnamespace=0&apfilterredir=nonredirects&aplimit=500&apdir=ascending';
