<x-guest-layout>
        ...
        <div class="container flex flex-wrap items-center justify-between w-full px-2 py-2 mx-auto mt-0 lg:py-6">
            ...
            <div :class="{'flex': open, 'hidden': ! open}" class="hidden w-full pt-2">
                <ul class="grid flex-1 space-y-1 list-reset lg:flex place-items-end justify-items-end">
                    <x-home.responsive-nav-link title="@lang('Blog')" url="{{ route('blog.index') }}">
                        <x-slot name="icon">
                    ...
                    @else
                        <x-home.responsive-nav-link title="@lang('Go to Dashboard')" url="{{ route('dashboard') }}">
                            <x-slot name="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="z-20 flex-grow hidden w-full p-4 mt-2 lg:flex lg:items-center lg:w-auto lg:mt-0 lg:p-0"
                ...
                <ul class="items-center justify-end flex-1 list-reset lg:flex">
                    <x-home.nav-link title="@lang('Blog')" url="{{ route('blog.index') }}">
                        <x-slot name="icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" />
                            </svg>
                    ...
                    @else
                        <x-home.nav-link title="@lang('Go to Dashboard')" url="{{ route('dashboard') }}">
                            <x-slot name="icon">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h14a1 1 0 011 1v2a1 1 0 01-1 1H5a1 1 0 01-1-1V5zM4 13a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H5a1 1 0 01-1-1v-6zM16 13a1 1 0 011-1h2a1 1 0 011 1v6a1 1 0 01-1 1h-2a1 1 0 01-1-1v-6z" />
                                </svg>
    ...
    <section class="py-12 bg-gray-100 border-b dark:bg-gray-900 dark:border-black">
        <div class="grid items-center justify-between max-w-full grid-flow-row grid-cols-1 gap-4 mx-auto text-4xl font-bold text-gray-900 opacity-75 md:text-5xl sm:grid-flow-auto sm:grid-cols-3 xl:grid-cols-6 dark:text-gray-200">
            ...
            <div class="flex flex-col items-center">
                ...
                <div class="flex text-lg text-gray-600 md:text-xl dark:text-gray-400">{{ App\Helpers\NumberHelper::nearestK($numberOfReviews) }} @lang('User Reviews')</div>
                {{-- USER REVIEWS OF THE GAME --}}
            </div>

            <div class="flex flex-col items-center">
                <div class="flex">{{ ucfirst(\App\Helpers\StatsHelper::getInGameSeason()) }}</div>
                <div class="flex text-lg text-gray-600 md:text-xl dark:text-gray-400">@lang('In-Game Season')</div>
                {{-- CURRENT IN-GAME SEASON --}}
            </div>

            <div class="flex flex-col items-center">
                <div class="flex">{{ App\Helpers\NumberHelper::nearestK(\App\Helpers\StatsHelper::countPlayers()) }}</div>
                <div class="flex text-lg text-gray-600 md:text-xl dark:text-gray-400">@lang('Online Players')</div>
                {{-- ONLINE USERS FROM THE SERVER --}}
            </div>

            <div class="flex flex-col items-center">
                <div class="flex">{{ App\Helpers\NumberHelper::nearestK(App\Models\User::count()) }}</div>
                <div class="flex text-lg text-gray-600 md:text-xl dark:text-gray-400">@lang('Active Users')</div>
                {{-- ACTIVE USERS FROM THE WEBSITE --}}
            </div>

            <div class="flex flex-col items-center">
                <div class="flex">{{ App\Helpers\NumberHelper::nearestK(\App\Helpers\DiscordHelper::countMembers()) }}</div>
                <div class="flex text-lg text-gray-600 md:text-xl dark:text-gray-400">@lang('Discord Users')</div>
                {{-- TOTAL COUNT DISCORD USERS --}}
            </div>

            <div class="flex flex-col items-center">
                <div class="flex">{{ App\Helpers\NumberHelper::nearestK(\App\Helpers\StatsHelper::countForumMembers()) }}</div>
                <div class="flex text-lg text-gray-600 md:text-xl dark:text-gray-400">@lang('Forum Users')</div>
