<x-modal>
    ...
    <x-slot name="content">
        <p><span class="text-gray-400">@lang('Latest supported version'):</span> {{ $update->game_version->version }}</p>
        <p><span class="text-gray-400">@lang('Downloads'):</span> {{ $update->downloads }}</p>
        <p><span class="text-gray-400">@lang('Released'):</span> {{ $update->created_at->diffForHumans() }}</p>
        <article class="pt-4 mt-4 prose border-t border-gray-400 dark:prose-invert dark:text-gray-100 prose-a:text-green-600 dark:border-gray-700">
