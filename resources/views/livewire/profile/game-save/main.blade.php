<div>
    ...
    <div wire:init="loadData">
        @if($gamejolt and $gamesave)
            <p class="text-slate-600 dark:text-slate-400 text-sm">{{ trans('Last synced') }}: {{ $gamesave->updated_at->diffForHumans() }}</p>
        ...
        @else
            <span wire:loading.remove class='text-slate-500 text-sm pt-2'>{{ trans('User has not connected their Game Jolt account yet') }}</span>
        @endif
    </div>
</div>
