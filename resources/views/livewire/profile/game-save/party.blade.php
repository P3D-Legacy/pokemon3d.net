<div x-cloak>
    ...
    <div class="flex flex-col space-y-4" wire:init="loadData">
        @forelse($party as $pokemon)
            <div class="flex flex-col items-center bg-white border border-slate-200 rounded-lg shadow md:flex-row dark:border-slate-700 dark:bg-slate-800">
                <img class="object-cover w-28 h-auto rounded-lg m-4" src="{{ $pokemon['Image'] }}" alt="{{ $pokemon['Pokemon'] }}">
                <div class="flex flex-col justify-between p-2 leading-normal">
                    <h5 class="mb-1 text-lg font-bold tracking-tight text-slate-900 dark:text-white">
                        @if($pokemon['isShiny'])
                        ...
                        @if($pokemon['EggSteps'] > 0)
                            ...
                            {{ __('Egg') }}
                        @else
                            {{ (isset($pokemon['NickName']) && $pokemon['NickName'] != '') ? $pokemon['NickName'] : $pokemon['PokemonName'] }} <span class="text-sm text-slate-400 dark:text-slate-600">&middot; {{ $pokemon['PokemonName'] }} #{{ $pokemon['Pokemon'] }}</span>
                        @endif
                    </h5>
                    <div class="text-slate-700 dark:text-slate-400 text-sm">
                        @if($pokemon['EggSteps'] > 0)
