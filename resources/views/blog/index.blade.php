        ...
        <div class="flex flex-col items-center min-h-screen pt-6 sm:pt-0">
			...
			<div class="w-full p-6 pt-4 mt-6 mb-10 overflow-hidden rounded-lg shadow-md sm:max-w-2xl bg-gray-50 dark:bg-gray-800">
				...
				<ul class="flex mb-8 text-sm text-gray-500 lg:text-base">
                    ...
                    <li class="inline-flex items-center">
                        <p class="text-gray-300">@lang('Blog')</p>
                    </li>
                </ul>
						...
						<div class="block w-full h-full">
							<div class="w-full p-4 bg-white dark:bg-gray-800">
								<p class="font-medium text-green-500 text-md">
									{{ now()->subYear(1) > $post->published_at ? $post->published_at->isoFormat('LL') : $post->published_at->diffForHumans() }}<span class="text-sm text-gray-400 dark:text-gray-500"> &middot; {{ read_time($post->body)}}{!! ($post->tags->count()) ? ' &middot; ' : '' !!}</span>
