            ...
            <div className="mx-auto w-full max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
                <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
                    <div className="flex flex-col gap-2">
                        ...
                        <h1 className="text-3xl font-semibold tracking-tight">{t('My Skins')}</h1>
                        <p className="text-sm text-muted-foreground">
                            {t('Manage your in-game skin and library uploads.')}
                        </p>
                    </div>
                    <div className="flex items-center gap-3">
                        <span className="text-sm text-muted-foreground">
                            {t('Slots')}: <span className="font-semibold text-foreground">{slots.used}</span> /{' '}
                            {slots.max}
                        </span>
                ...
                <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    <div className="space-y-4">
                            ...
                            {currentSkin.exists && currentSkin.image_url ? (
                                <>
                                    <SkinImage
                                        ...
                                        alt={auth.user?.username ?? t('Current skin')}
                                        width={96}
                                        height={128}
                                    />
                                    <div className="mt-2 flex flex-wrap gap-2">
                                        ...
                                        <Button
                                            ...
                                            variant="destructive"
                                            onClick={() => {
                                                if (confirm(t('Delete your in-game skin?'))) {
                                ...
                                <p className="text-sm text-muted-foreground">
                                    {t('No skins have been added to your account yet.')}
                                </p>
                            )}
                        </section>
                        ...
                        {deleteActivity.length > 0 && (
                            <section className="border border-border bg-card p-4">
                                <h2 className="mb-3 text-lg font-semibold tracking-tight">
                                    {t('Admin Skin Deletion Activity')}
                                </h2>
                        ...
                        <section className="border border-border bg-card p-4">
                            <h2 className="mb-3 text-lg font-semibold tracking-tight">{t('Skin Information')}</h2>
                            <div className="space-y-3 text-sm text-muted-foreground">
                                <p>
                                    {t('Want to make your own skin?')}{' '}
                                ...
                                <div>
                                    <h3 className="font-medium text-foreground">{t('File validation')}</h3>
                                    <ul className="mt-1 list-disc space-y-1 pl-5">
                                        <li>{t('Less than 2MB')}</li>
                                        <li>{t('Must be a PNG file')}</li>
                                        <li>
                                            {t('Exact dimensions of :width×:height', {
                                                width,
                                                height,
                                            })}
                                        </li>
                                ...
                                <div>
                                    <h3 className="font-medium text-foreground">{t('Rules')}</h3>
                                    <ul className="mt-1 list-disc space-y-1 pl-5">
                                        <li>
                                            {t(
                                                'Every part (for a :width×:height sprite, every 32×32 portion) of the skin has to contain at least one pixel that is not transparent.',
                                                { width, height },
                                            )}
                                        </li>
                                        <li>{t('You have to own the rights to use the image you upload.')}</li>
                                        <li>{t('The image must not contain any sexual or harassing content.')}</li>
                                    </ul>
                                </div>
                                <p className="text-xs">
                                    {t(
                    ...
                    <div className="lg:col-span-2">
                        {skins.length === 0 ? (
                            <div className="flex flex-col items-center justify-center gap-3 border border-border bg-muted/20 px-6 py-16 text-center">
                                ...
                                <div className="text-lg font-medium">{t('No skins yet')}</div>
                                <p className="max-w-md text-sm text-muted-foreground">
                                    {t('Upload a skin to fill one of your :max slots.', { max: slots.max })}
                                </p>
                                <Button variant="default" size="sm" asChild>
                                    <Link href={create.url()}>
                                        <PlusIcon data-icon="inline-start" weight="bold" />
